...
class TikTokClientV2Upload
{
	...
	public function curl_post_put($url, $postFields = null, $fileFields = null, $headerFields = null, $method)
	{
		...
		$tempFiles = array();
		$payload = $postFields ?: array();
		if ($hasFiles) {
			$payload = is_array($payload) ? $payload : array('data' => json_encode($postFields));
			foreach ($fileFields as $name => $file) {
				$payload[$name] = $this->buildCurlFile($file, $tempFiles);
			}
		} else {
	...
	protected function buildCurlFile($file, array &$tempFiles)
	{
		...
		if (isset($file['path']) && is_string($file['path']) && $file['path'] !== '') {
			return curl_file_create($file['path'], $mimeType, $fileName);
		}

		$content = isset($file['content']) ? $file['content'] : '';
		$tempFile = tempnam(sys_get_temp_dir(), 'tiktok_upload_');
		if ($tempFile === false) {
			throw new Exception('Unable to create temporary file for upload');
		}

		file_put_contents($tempFile, $content);
		$tempFiles[] = $tempFile;

		return curl_file_create($tempFile, $mimeType, $fileName);
	}

	protected function cleanupTempFiles(array $tempFiles)
	{
